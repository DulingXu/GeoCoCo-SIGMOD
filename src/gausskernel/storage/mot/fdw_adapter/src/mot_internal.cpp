...
// GeoGauss Group Management Configuration
// 分组配置：节点分组管理
struct GroupConfig {
    uint64_t group_id;
    bool is_leader;
...
// 全局分组配置
static std::vector<GroupConfig> g_group_configs;
static std::map<uint64_t, uint64_t> g_node_to_group_map;  // 节点ID到组ID的映射
static std::map<uint64_t, std::vector<uint64_t>> g_group_to_nodes_map;  // 组ID到节点列表的映射
static std::map<uint64_t, uint64_t> g_group_leaders_map;  // 组ID到组长ID的映射

// 分组相关常量
static const uint64_t GROUP_INTERNAL_PORT_BASE = 6000;  // 组内通信端口基础
static const uint64_t GROUP_EXTERNAL_PORT_BASE = 6100;  // 组间通信端口基础

// 初始化分组配置
// 示例：5个节点(0,1,2,3,4)分成2-2-1三组
void InitGroupConfig() {
    g_group_configs.clear();
    g_node_to_group_map.clear();
    g_group_to_nodes_map.clear();
    g_group_leaders_map.clear();
    
    // 3个节点分组配置：
    // 组0：节点0(leader), 节点1(member)
    GroupConfig group0;
    group0.group_id = 0;
    group0.leader_id = 0;
    group0.members = {0, 1};
    g_group_configs.push_back(group0);
    ...
    group1.leader_id = 2;
    group1.members = {2};
    g_group_configs.push_back(group1);
    
    // 构建映射关系
    for (const auto& group : g_group_configs) {
        g_group_leaders_map[group.group_id] = group.leader_id;
        g_group_to_nodes_map[group.group_id] = group.members;
        
        for (uint64_t node_id : group.members) {
            g_node_to_group_map[node_id] = group.group_id;
        }
    }
}

// 获取节点所在组ID
uint64_t GetNodeGroupId(uint64_t node_id) {
    auto it = g_node_to_group_map.find(node_id);
    return (it != g_node_to_group_map.end()) ? it->second : UINT64_MAX;
}

// 判断节点是否为组长
bool IsGroupLeader(uint64_t node_id) {
    uint64_t group_id = GetNodeGroupId(node_id);
    if (group_id == UINT64_MAX) return false;
    
    auto it = g_group_leaders_map.find(group_id);
    return (it != g_group_leaders_map.end()) && (it->second == node_id);
}

// 获取组长ID
uint64_t GetGroupLeader(uint64_t group_id) {
    auto it = g_group_leaders_map.find(group_id);
    return (it != g_group_leaders_map.end()) ? it->second : UINT64_MAX;
}

// 获取组内成员列表
std::vector<uint64_t> GetGroupMembers(uint64_t group_id) {
    auto it = g_group_to_nodes_map.find(group_id);
    return (it != g_group_to_nodes_map.end()) ? it->second : std::vector<uint64_t>();
}

// 获取所有组长节点列表
std::vector<uint64_t> GetAllGroupLeaders() {
    std::vector<uint64_t> leaders;
    for (const auto& pair : g_group_leaders_map) {
...
// 获取当前节点的组长ID
uint64_t GetCurrentNodeGroupLeader() {
    uint64_t group_id = GetNodeGroupId(local_ip_index);
    return GetGroupLeader(group_id);
}

// 判断两个节点是否在同一组
bool IsInSameGroup(uint64_t node1, uint64_t node2) {
    return GetNodeGroupId(node1) == GetNodeGroupId(node2);
}
...
// 前向声明分组相关函数
void SendToGroupLeader(std::string* serialized_str_ptr);
void SendToOtherGroupLeaders(std::string* serialized_str_ptr);
void IntraGroupSendThreadMain(uint64_t id);
void InterGroupSendThreadMain(uint64_t id);
void IntraGroupListenThreadMain(uint64_t id);
void InterGroupListenThreadMain(uint64_t id);
void IntraGroupMergeThreadMain(uint64_t id);
void InterGroupMergeThreadMain(uint64_t id);
...
// 分组通信相关的队列
BlockingConcurrentQueue<std::unique_ptr<send_thread_params>> intra_group_send_pool;  // 组内通信池
BlockingConcurrentQueue<std::unique_ptr<send_thread_params>> inter_group_send_pool;  // 组间通信池
BlockingConcurrentQueue<std::unique_ptr<zmq::message_t>> intra_group_listen_queue;   // 组内监听队列
BlockingConcurrentQueue<std::unique_ptr<zmq::message_t>> inter_group_listen_queue;   // 组间监听队列
...
bool MOTAdaptor::InsertTxntoLocalChangeSet(MOT::TxnManager* txMan, const uint64_t& index_pack, const uint64_t& index_unique){
    ...
    if(kServerNum > 1) {
        ...
        // 使用分组发送：组员发送给组长，组长直接处理本地数据
        SendToGroupLeader(serialized_txn_str_ptr);
...
void InitEpochTimerManager(){
    ...
    // 初始化分组配置
    InitGroupConfig();
    MOT_LOG_INFO("节点%llu分组初始化完成，是否为组长: %s", local_ip_index, IsGroupLeader(local_ip_index) ? "是" : "否");
    ...
    // 启动分组相关的线程
    std::thread intra_group_send_thread(IntraGroupSendThreadMain, 0);
    intra_group_send_thread.detach();
    
    std::thread inter_group_send_thread(InterGroupSendThreadMain, 0);
    inter_group_send_thread.detach();
    
    // 只有组长需要启动监听和merge线程
    if (IsGroupLeader(local_ip_index)) {
        std::thread intra_group_listen_thread(IntraGroupListenThreadMain, 0);
        intra_group_listen_thread.detach();
        
        std::thread inter_group_listen_thread(InterGroupListenThreadMain, 0);
        inter_group_listen_thread.detach();
        
        std::thread intra_group_merge_thread(IntraGroupMergeThreadMain, 0);
        intra_group_merge_thread.detach();
        
        std::thread inter_group_merge_thread(InterGroupMergeThreadMain, 0);
        inter_group_merge_thread.detach();
        
        MOT_LOG_INFO("组长节点%llu启动分组通信线程", local_ip_index);
    } else {
        MOT_LOG_INFO("组员节点%llu启动发送线程", local_ip_index);
    }
}
...
void SendToGroupLeader(std::string* serialized_str_ptr) {
    if (IsGroupLeader(local_ip_index)) {
        ...
        auto message_string_ptr = std::make_unique<std::string>(*serialized_str_ptr);
        ...
        // 如果是组员，发送给组长
        uint64_t leader_id = GetCurrentNodeGroupLeader();
        if (leader_id != UINT64_MAX) {
            // 使用组内通信池发送
            intra_group_send_pool.enqueue(std::make_unique<send_thread_params>(0, leader_id, serialized_str_ptr));
            intra_group_send_pool.enqueue(std::make_unique<send_thread_params>(0, 0, std::move(nullptr)));
        }
    }
}
...
void SendToOtherGroupLeaders(std::string* serialized_str_ptr) {
    if (IsGroupLeader(local_ip_index)) {
        // 只有组长才能发送给其他组长
        auto leaders = GetAllGroupLeaders();
        for (uint64_t leader_id : leaders) {
            if (leader_id != local_ip_index) {
                // 使用组间通信池发送
                inter_group_send_pool.enqueue(std::make_unique<send_thread_params>(0, leader_id, serialized_str_ptr));
                inter_group_send_pool.enqueue(std::make_unique<send_thread_params>(0, 0, std::move(nullptr)));
            }
        }
    }
}
...
void EpochPackThreadMain(uint64_t id){
    ...
    SetCPU();
    uint64_t current_epoch = 0, index = id, send_epoch = 1;
    ...
    while(true){
        ...
        if(id == 0) {
            while(MOTAdaptor::IsCurrentEpochFinished(send_epoch) == true) {
                ...
                if(!send_pool.enqueue(std::move(std::make_unique<send_thread_params>(0, 0, std::move(nullptr)))) ) Assert(false);
                MOT_LOG_INFO("MergeRequest 到达发送时间，开始发送 第%llu个Pack线程, epoch:%llu 共%llu : %llu", 
                    id, send_epoch, MOTAdaptor::LoadPackedTxnNum(send_epoch), MOTAdaptor::LoadChangeSet(send_epoch));
...
void IntraGroupSendThreadMain(uint64_t id) {
    SetCPU();
    MOT_LOG_INFO("IntraGroupSendThread 开始工作");
    std::unique_ptr<send_thread_params> params;
    std::unique_ptr<zmq::message_t> msg;
    std::vector<std::unique_ptr<zmq::context_t>> contexts;
    std::vector<std::unique_ptr<zmq::socket_t>> sockets;
    ...
    for(int i = 0; i < (int) kServerNum; i++) {
        contexts.emplace_back(std::move(std::make_unique<zmq::context_t>(1)));
        sockets.emplace_back(std::move(std::make_unique<zmq::socket_t>(*contexts[i], ZMQ_PUSH)));
        uint64_t port = GROUP_INTERNAL_PORT_BASE + i;
        sockets[i]->connect("tcp://" + kServerIp[i] + ":" + std::to_string(port));
    }
    
    while(init_ok.load() == false) usleep(200);
    for(;;) {
        intra_group_send_pool.wait_dequeue(params);
        if(params == nullptr || params->merge_request_ptr == nullptr) continue;
        
        MOT_LOG_INFO("发送组内消息到组长 %llu", params->tot);
        msg = std::make_unique<zmq::message_t>(static_cast<void*>(const_cast<char*>(params->merge_request_ptr->data())),
                params->merge_request_ptr->size(), string_free, static_cast<void*>(params->merge_request_ptr));
        sockets[params->tot]->send(*(msg));
    }
...
void InterGroupSendThreadMain(uint64_t id) {
    SetCPU();
    MOT_LOG_INFO("InterGroupSendThread 开始工作");
    std::unique_ptr<send_thread_params> params;
    std::unique_ptr<zmq::message_t> msg;
    std::vector<std::unique_ptr<zmq::context_t>> contexts;
    std::vector<std::unique_ptr<zmq::socket_t>> sockets;
    ...
    for(int i = 0; i < (int) kServerNum; i++) {
        contexts.emplace_back(std::move(std::make_unique<zmq::context_t>(1)));
        sockets.emplace_back(std::move(std::make_unique<zmq::socket_t>(*contexts[i], ZMQ_PUSH)));
        uint64_t port = GROUP_EXTERNAL_PORT_BASE + i;
        sockets[i]->connect("tcp://" + kServerIp[i] + ":" + std::to_string(port));
    }
    
    while(init_ok.load() == false) usleep(200);
    for(;;) {
        inter_group_send_pool.wait_dequeue(params);
        if(params == nullptr || params->merge_request_ptr == nullptr) continue;
        
        MOT_LOG_INFO("发送组间消息到组长 %llu", params->tot);
        msg = std::make_unique<zmq::message_t>(static_cast<void*>(const_cast<char*>(params->merge_request_ptr->data())),
                params->merge_request_ptr->size(), string_free, static_cast<void*>(params->merge_request_ptr));
        sockets[params->tot]->send(*(msg));
    }
...
void IntraGroupListenThreadMain(uint64_t id) {
    SetCPU();
    MOT_LOG_INFO("IntraGroupListenThread 开始工作");
    zmq::context_t listen_context(1);
    zmq::socket_t socket_listen(listen_context, ZMQ_PULL);
    
    // 组长监听组内通信端口
    uint64_t port = GROUP_INTERNAL_PORT_BASE + local_ip_index;
    ...
    for (;;) {
        ...
        socket_listen.recv(&(*message_ptr));
        if(is_epoch_advance_started.load() == true){
            if(!intra_group_listen_queue.enqueue(std::move(message_ptr))) assert(false);
            if(!intra_group_listen_queue.enqueue(std::move(std::make_unique<zmq::message_t>()))) assert(false);
            MOT_LOG_INFO("IntraGroup 接收到第一条消息");
            break;
        }
    }
...
void InterGroupListenThreadMain(uint64_t id) {
    SetCPU();
    MOT_LOG_INFO("InterGroupListenThread 开始工作");
    zmq::context_t listen_context(1);
    zmq::socket_t socket_listen(listen_context, ZMQ_PULL);
    
    // 组长监听组间通信端口
    uint64_t port = GROUP_EXTERNAL_PORT_BASE + local_ip_index;
    socket_listen.bind("tcp://*:" + std::to_string(port));
    ...
    for (;;) {
        ...
        socket_listen.recv(&(*message_ptr));
        if(is_epoch_advance_started.load() == true){
            if(!inter_group_listen_queue.enqueue(std::move(message_ptr))) assert(false);
            if(!inter_group_listen_queue.enqueue(std::move(std::make_unique<zmq::message_t>()))) assert(false);
            MOT_LOG_INFO("InterGroup 接收到第一条消息");
            break;
        }
    }
...
void EpochMergeThreadMain(uint64_t id){
    ...
    uint64_t csn = 0, index_pack = id % kPackageNum, epoch_mod = 0, server_id = 0, clear_epoch = 0, epoch = 0, received_pack_num = 0;
    uint32_t op_type = 0;
    int KeyLength;
    void* buf;
...
// 组内merge线程：组长处理组员的事务
void IntraGroupMergeThreadMain(uint64_t id) {
    MOT_LOG_INFO("IntraGroupMergeThread 开始工作 节点%llu", local_ip_index);
    
    // 只有组长才运行此线程
    if (!IsGroupLeader(local_ip_index)) {
        MOT_LOG_INFO("节点%llu不是组长，IntraGroupMergeThread退出", local_ip_index);
        return;
    }
    ...
    std::unique_ptr<std::vector<MOT::Row*>> row_vector_ptr;
    std::string csn_temp, key_temp, key_str, table_name, csn_result;
    uint64_t csn = 0, epoch_mod = 0, server_id = 0;
    uint32_t op_type = 0;
    int KeyLength;
    void* buf;
    bool result = false;
    
    // 存储组内合并后的数据，用于后续组间merge
    std::vector<std::unique_ptr<merge::Message>> intra_group_merged_data;
    
    while(!init_ok.load()) usleep(200);
    
    for(;;) {
        // 监听组内消息
        if(intra_group_listen_queue.try_dequeue(message_ptr) && message_ptr != nullptr && message_ptr->size() > 0) {
            msg_ptr = std::make_unique<merge::Message>();
            auto message_string_ptr = std::make_unique<std::string>(static_cast<const char*>(message_ptr->data()), message_ptr->size());
            google::protobuf::io::ArrayInputStream inputStream(message_string_ptr->data(), message_string_ptr->size());
            google::protobuf::io::GzipInputStream gzipStream(&inputStream);
            msg_ptr->ParseFromZeroCopyStream(&gzipStream);
            ...
            if(msg_ptr->type_case() == merge::Message::TypeCase::kTxn) {
                auto& txn = msg_ptr->txn();
                MOT_LOG_INFO("组长%llu处理来自组员%llu的事务", local_ip_index, txn.server_id());
                ...
                if(!result) {
                    MOTAdaptor::abort_transcation_csn_set.insert(csn_temp, csn_temp);
                    MOT_LOG_INFO("组内事务merge失败，事务被中止");
                }
                else{
                    // 创建消息的副本用于组间发送
                    auto msg_copy = std::make_unique<merge::Message>(*msg_ptr);
                    intra_group_merged_data.push_back(std::move(msg_copy));
                    
                    // 提交到commit队列（使用原始消息）
                    if(commit_txn_queue_struct.enqueue(std::make_unique<commit_thread_params>(std::move(msg_ptr), std::move(row_vector_ptr)))){
                        MOTAdaptor::IncRemoteCommitTxnCounters(epoch_mod, id % kPackageNum);
                        if(!commit_txn_queue_struct.enqueue(std::make_unique<commit_thread_params>(nullptr, nullptr))) Assert(false);
                    }
                    else{
                        MOT_LOG_INFO("组内Merge 放入队列失败");
                        Assert(false);
                    }
                }
                MOTAdaptor::IncRemoteMergedTxnCounters(epoch_mod, id % kPackageNum);
            }
        }
        
        // 检查是否需要向其他组长发送组内merge后的数据
        if (!intra_group_merged_data.empty()) {
            // 这里可以定期或根据条件向其他组长发送数据
            // 为了简化，这里直接发送
            for (auto& merged_msg : intra_group_merged_data) {
                auto serialized_str_ptr = Gzip(std::move(merged_msg));
                SendToOtherGroupLeaders(serialized_str_ptr);
            }
            intra_group_merged_data.clear();
        }
...
// 组间merge线程：组长之间的事务merge
void InterGroupMergeThreadMain(uint64_t id) {
    MOT_LOG_INFO("InterGroupMergeThread 开始工作 节点%llu", local_ip_index);
    
    // 只有组长才运行此线程
    if (!IsGroupLeader(local_ip_index)) {
        MOT_LOG_INFO("节点%llu不是组长，InterGroupMergeThread退出", local_ip_index);
        return;
    }
    ...
    for(;;) {
        // 监听组间消息
        if(inter_group_listen_queue.try_dequeue(message_ptr) && message_ptr != nullptr && message_ptr->size() > 0) {
            msg_ptr = std::make_unique<merge::Message>();
            auto message_string_ptr = std::make_unique<std::string>(static_cast<const char*>(message_ptr->data()), message_ptr->size());
            google::protobuf::io::ArrayInputStream inputStream(message_string_ptr->data(), message_string_ptr->size());
            google::protobuf::io::GzipInputStream gzipStream(&inputStream);
            ...
            if(msg_ptr->type_case() == merge::Message::TypeCase::kTxn) {
                auto& txn = msg_ptr->txn();
                MOT_LOG_INFO("组长%llu处理来自其他组长%llu的事务", local_ip_index, txn.server_id());
                ...
                if(!result) {
                    MOTAdaptor::abort_transcation_csn_set.insert(csn_temp, csn_temp);
                    MOT_LOG_INFO("组间事务merge失败，事务被中止");
                }
                else{
                    // 提交到commit队列
                    if(commit_txn_queue_struct.enqueue(std::make_unique<commit_thread_params>(std::move(msg_ptr), std::move(row_vector_ptr)))){
